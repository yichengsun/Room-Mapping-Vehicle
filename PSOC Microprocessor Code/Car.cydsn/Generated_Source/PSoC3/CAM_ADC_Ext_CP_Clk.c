/*******************************************************************************
* File Name: CAM_ADC_Ext_CP_Clk.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CAM_ADC_Ext_CP_Clk_Start(void) 
{
    /* Set the bit to enable the clock. */
    CAM_ADC_Ext_CP_Clk_CLKEN |= CAM_ADC_Ext_CP_Clk_CLKEN_MASK;
	CAM_ADC_Ext_CP_Clk_CLKSTBY |= CAM_ADC_Ext_CP_Clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CAM_ADC_Ext_CP_Clk_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    CAM_ADC_Ext_CP_Clk_CLKEN &= (uint8)(~CAM_ADC_Ext_CP_Clk_CLKEN_MASK);
	CAM_ADC_Ext_CP_Clk_CLKSTBY &= (uint8)(~CAM_ADC_Ext_CP_Clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CAM_ADC_Ext_CP_Clk_StopBlock(void) 
{
    if ((CAM_ADC_Ext_CP_Clk_CLKEN & CAM_ADC_Ext_CP_Clk_CLKEN_MASK) != 0u)
    {
...
#if defined(CAM_ADC_Ext_CP_Clk__CFG3)
        CLK_DIST_AMASK = CAM_ADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CAM_ADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CAM_ADC_Ext_CP_Clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CAM_ADC_Ext_CP_Clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CAM_ADC_Ext_CP_Clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CAM_ADC_Ext_CP_Clk_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        CAM_ADC_Ext_CP_Clk_CLKSTBY &= (uint8)(~CAM_ADC_Ext_CP_Clk_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CAM_ADC_Ext_CP_Clk_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = CAM_ADC_Ext_CP_Clk_GetSourceRegister();
    uint16 oldDivider = CAM_ADC_Ext_CP_Clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CAM_ADC_Ext_CP_Clk_CLKEN & CAM_ADC_Ext_CP_Clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CAM_ADC_Ext_CP_Clk_DIV_PTR, clkDivider);
                CAM_ADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CAM_ADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CAM_ADC_Ext_CP_Clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CAM_ADC_Ext_CP_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(CAM_ADC_Ext_CP_Clk_DIV_PTR, clkDivider);
				CAM_ADC_Ext_CP_Clk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 CAM_ADC_Ext_CP_Clk_GetDividerRegister(void) 
{
    return CY_GET_REG16(CAM_ADC_Ext_CP_Clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CAM_ADC_Ext_CP_Clk_SetModeRegister(uint8 modeBitMask) 
{
    CAM_ADC_Ext_CP_Clk_MOD_SRC |= modeBitMask & (uint8)CAM_ADC_Ext_CP_Clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CAM_ADC_Ext_CP_Clk_ClearModeRegister(uint8 modeBitMask) 
{
    CAM_ADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CAM_ADC_Ext_CP_Clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 CAM_ADC_Ext_CP_Clk_GetModeRegister(void) 
{
    return CAM_ADC_Ext_CP_Clk_MOD_SRC & (uint8)(CAM_ADC_Ext_CP_Clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CAM_ADC_Ext_CP_Clk_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = CAM_ADC_Ext_CP_Clk_GetDividerRegister();
    uint8 oldSrc = CAM_ADC_Ext_CP_Clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CAM_ADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
        CAM_ADC_Ext_CP_Clk_MOD_SRC =
            (CAM_ADC_Ext_CP_Clk_MOD_SRC & (uint8)(~CAM_ADC_Ext_CP_Clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 CAM_ADC_Ext_CP_Clk_GetSourceRegister(void) 
{
    return CAM_ADC_Ext_CP_Clk_MOD_SRC & CAM_ADC_Ext_CP_Clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CAM_ADC_Ext_CP_Clk_SetPhaseRegister(uint8 clkPhase) 
{
    CAM_ADC_Ext_CP_Clk_PHASE = clkPhase & CAM_ADC_Ext_CP_Clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CAM_ADC_Ext_CP_Clk_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 CAM_ADC_Ext_CP_Clk_GetPhaseRegister(void) 
{
    return CAM_ADC_Ext_CP_Clk_PHASE & CAM_ADC_Ext_CP_Clk_PHASE_MASK;
}
