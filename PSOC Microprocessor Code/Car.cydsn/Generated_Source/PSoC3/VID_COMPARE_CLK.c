...
/*******************************************************************************
* Function Name: VID_COMPARE_CLK_Start
********************************************************************************
*
* Summary:
...
void VID_COMPARE_CLK_Start(void) 
{
    ...
    VID_COMPARE_CLK_CLKEN |= VID_COMPARE_CLK_CLKEN_MASK;
	VID_COMPARE_CLK_CLKSTBY |= VID_COMPARE_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: VID_COMPARE_CLK_Stop
********************************************************************************
*
* Summary:
...
void VID_COMPARE_CLK_Stop(void) 
{
    ...
    VID_COMPARE_CLK_CLKEN &= (uint8)(~VID_COMPARE_CLK_CLKEN_MASK);
	VID_COMPARE_CLK_CLKSTBY &= (uint8)(~VID_COMPARE_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: VID_COMPARE_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((VID_COMPARE_CLK_CLKEN & VID_COMPARE_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = VID_COMPARE_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = VID_COMPARE_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* VID_COMPARE_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(VID_COMPARE_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(VID_COMPARE_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: VID_COMPARE_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: VID_COMPARE_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = VID_COMPARE_CLK_GetSourceRegister();
    uint16 oldDivider = VID_COMPARE_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = VID_COMPARE_CLK_CLKEN & VID_COMPARE_CLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(VID_COMPARE_CLK_DIV_PTR, clkDivider);
                VID_COMPARE_CLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                VID_COMPARE_CLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(VID_COMPARE_CLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((VID_COMPARE_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: VID_COMPARE_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(VID_COMPARE_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: VID_COMPARE_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    VID_COMPARE_CLK_MOD_SRC |= modeBitMask & (uint8)VID_COMPARE_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: VID_COMPARE_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    VID_COMPARE_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(VID_COMPARE_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: VID_COMPARE_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return VID_COMPARE_CLK_MOD_SRC & (uint8)(VID_COMPARE_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: VID_COMPARE_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = VID_COMPARE_CLK_GetDividerRegister();
    uint8 oldSrc = VID_COMPARE_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        VID_COMPARE_CLK_MOD_SRC |= CYCLK_SSS;
        VID_COMPARE_CLK_MOD_SRC =
            (VID_COMPARE_CLK_MOD_SRC & (uint8)(~VID_COMPARE_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: VID_COMPARE_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return VID_COMPARE_CLK_MOD_SRC & VID_COMPARE_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: VID_COMPARE_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    VID_COMPARE_CLK_PHASE = clkPhase & VID_COMPARE_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: VID_COMPARE_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return VID_COMPARE_CLK_PHASE & VID_COMPARE_CLK_PHASE_MASK;
}
