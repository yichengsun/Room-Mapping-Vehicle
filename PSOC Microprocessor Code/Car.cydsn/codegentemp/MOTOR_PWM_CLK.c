...
/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_Start
********************************************************************************
*
* Summary:
...
void MOTOR_PWM_CLK_Start(void) 
{
    ...
    MOTOR_PWM_CLK_CLKEN |= MOTOR_PWM_CLK_CLKEN_MASK;
	MOTOR_PWM_CLK_CLKSTBY |= MOTOR_PWM_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_Stop
********************************************************************************
*
* Summary:
...
void MOTOR_PWM_CLK_Stop(void) 
{
    ...
    MOTOR_PWM_CLK_CLKEN &= (uint8)(~MOTOR_PWM_CLK_CLKEN_MASK);
	MOTOR_PWM_CLK_CLKSTBY &= (uint8)(~MOTOR_PWM_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((MOTOR_PWM_CLK_CLKEN & MOTOR_PWM_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = MOTOR_PWM_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = MOTOR_PWM_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* MOTOR_PWM_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(MOTOR_PWM_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(MOTOR_PWM_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = MOTOR_PWM_CLK_GetSourceRegister();
    uint16 oldDivider = MOTOR_PWM_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = MOTOR_PWM_CLK_CLKEN & MOTOR_PWM_CLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(MOTOR_PWM_CLK_DIV_PTR, clkDivider);
                MOTOR_PWM_CLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                MOTOR_PWM_CLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(MOTOR_PWM_CLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((MOTOR_PWM_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(MOTOR_PWM_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    MOTOR_PWM_CLK_MOD_SRC |= modeBitMask & (uint8)MOTOR_PWM_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    MOTOR_PWM_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(MOTOR_PWM_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return MOTOR_PWM_CLK_MOD_SRC & (uint8)(MOTOR_PWM_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = MOTOR_PWM_CLK_GetDividerRegister();
    uint8 oldSrc = MOTOR_PWM_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        MOTOR_PWM_CLK_MOD_SRC |= CYCLK_SSS;
        MOTOR_PWM_CLK_MOD_SRC =
            (MOTOR_PWM_CLK_MOD_SRC & (uint8)(~MOTOR_PWM_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return MOTOR_PWM_CLK_MOD_SRC & MOTOR_PWM_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    MOTOR_PWM_CLK_PHASE = clkPhase & MOTOR_PWM_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: MOTOR_PWM_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return MOTOR_PWM_CLK_PHASE & MOTOR_PWM_CLK_PHASE_MASK;
}
