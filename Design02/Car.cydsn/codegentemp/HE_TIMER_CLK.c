...
/*******************************************************************************
* Function Name: HE_TIMER_CLK_Start
********************************************************************************
*
* Summary:
...
void HE_TIMER_CLK_Start(void) 
{
    ...
    HE_TIMER_CLK_CLKEN |= HE_TIMER_CLK_CLKEN_MASK;
	HE_TIMER_CLK_CLKSTBY |= HE_TIMER_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HE_TIMER_CLK_Stop
********************************************************************************
*
* Summary:
...
void HE_TIMER_CLK_Stop(void) 
{
    ...
    HE_TIMER_CLK_CLKEN &= (uint8)(~HE_TIMER_CLK_CLKEN_MASK);
	HE_TIMER_CLK_CLKSTBY &= (uint8)(~HE_TIMER_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HE_TIMER_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HE_TIMER_CLK_CLKEN & HE_TIMER_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HE_TIMER_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HE_TIMER_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HE_TIMER_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HE_TIMER_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HE_TIMER_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HE_TIMER_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HE_TIMER_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HE_TIMER_CLK_GetSourceRegister();
    uint16 oldDivider = HE_TIMER_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HE_TIMER_CLK_CLKEN & HE_TIMER_CLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(HE_TIMER_CLK_DIV_PTR, clkDivider);
                HE_TIMER_CLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                HE_TIMER_CLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(HE_TIMER_CLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HE_TIMER_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HE_TIMER_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HE_TIMER_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: HE_TIMER_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HE_TIMER_CLK_MOD_SRC |= modeBitMask & (uint8)HE_TIMER_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: HE_TIMER_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HE_TIMER_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HE_TIMER_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: HE_TIMER_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HE_TIMER_CLK_MOD_SRC & (uint8)(HE_TIMER_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: HE_TIMER_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HE_TIMER_CLK_GetDividerRegister();
    uint8 oldSrc = HE_TIMER_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HE_TIMER_CLK_MOD_SRC |= CYCLK_SSS;
        HE_TIMER_CLK_MOD_SRC =
            (HE_TIMER_CLK_MOD_SRC & (uint8)(~HE_TIMER_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HE_TIMER_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HE_TIMER_CLK_MOD_SRC & HE_TIMER_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HE_TIMER_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HE_TIMER_CLK_PHASE = clkPhase & HE_TIMER_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HE_TIMER_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HE_TIMER_CLK_PHASE & HE_TIMER_CLK_PHASE_MASK;
}
