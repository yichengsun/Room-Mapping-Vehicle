...
uint8 VID_COMPARE_initVar = 0u;

/* Internal functions definitoin */
static void VID_COMPARE_trimAdjust(uint8 nibble) ;
...
/*******************************************************************************
* Function Name: VID_COMPARE_Init
********************************************************************************
*
* Summary:
...
void VID_COMPARE_Init(void) 
{
    ...
    VID_COMPARE_SetSpeed(VID_COMPARE_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( VID_COMPARE_DEFAULT_HYSTERESIS == 0u )
        VID_COMPARE_CR |= VID_COMPARE_HYST_OFF;
    #else
    ...
    /* Power down override feature is not supported for PSoC5A. */
        ...
        #if ( VID_COMPARE_DEFAULT_PWRDWN_OVRD == 0u )
            VID_COMPARE_CR &= (uint8)(~VID_COMPARE_PWRDWN_OVRD);
        #else 
    ...
    VID_COMPARE_CR |= VID_COMPARE_MX_AO;

    /* Set default sync */
    VID_COMPARE_CLK &= (uint8)(~VID_COMPARE_SYNCCLK_MASK);
    #if ( VID_COMPARE_DEFAULT_BYPASS_SYNC == 0u )
        VID_COMPARE_CLK |= VID_COMPARE_SYNC_CLK_EN;
    #else
...
/*******************************************************************************
* Function Name: VID_COMPARE_Enable
********************************************************************************
*
* Summary:
...
{
    VID_COMPARE_PWRMGR |= VID_COMPARE_ACT_PWR_EN;
    VID_COMPARE_STBY_PWRMGR |= VID_COMPARE_STBY_PWR_EN;
...
/*******************************************************************************
* Function Name: VID_COMPARE_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  VID_COMPARE_initVar: Is modified when this function is called for the 
...
void VID_COMPARE_Start(void) 
{
    ...
    if ( VID_COMPARE_initVar == 0u )
    {
        VID_COMPARE_Init();
        
        VID_COMPARE_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: VID_COMPARE_Stop
********************************************************************************
*
* Summary:
...
void VID_COMPARE_Stop(void) 
{
    ...
    VID_COMPARE_PWRMGR &= (uint8)(~VID_COMPARE_ACT_PWR_EN);
    VID_COMPARE_STBY_PWRMGR &= (uint8)(~VID_COMPARE_STBY_PWR_EN);
...
/*******************************************************************************
* Function Name: VID_COMPARE_SetSpeed
********************************************************************************
*
* Summary:
...
void VID_COMPARE_SetSpeed(uint8 speed) 
{
    ...
    VID_COMPARE_CR = (VID_COMPARE_CR & (uint8)(~VID_COMPARE_PWR_MODE_MASK)) |
                           (speed & VID_COMPARE_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == VID_COMPARE_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
        ...
        #if (CY_PSOC3 || CY_PSOC5LP) 
            VID_COMPARE_TR0 = VID_COMPARE_HS_TRIM_TR0;
            VID_COMPARE_TR1 = VID_COMPARE_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        ...
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            VID_COMPARE_TR0 = VID_COMPARE_LS_TRIM_TR0;
            VID_COMPARE_TR1 = VID_COMPARE_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: VID_COMPARE_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( VID_COMPARE_WRK & VID_COMPARE_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: VID_COMPARE_trimAdjust
********************************************************************************
*
* Summary:
...
static void VID_COMPARE_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = VID_COMPARE_WRK & VID_COMPARE_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            ...
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                VID_COMPARE_TR0 |= VID_COMPARE_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            ...
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                VID_COMPARE_TR1 |= VID_COMPARE_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((VID_COMPARE_WRK & VID_COMPARE_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: VID_COMPARE_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 VID_COMPARE_ZeroCal(void) 
{
    ...
    tmpSW0 = VID_COMPARE_SW0;
    tmpSW2 = VID_COMPARE_SW2;
    tmpSW3 = VID_COMPARE_SW3;

     /* Clear routing for inP, retain routing for inN */
    VID_COMPARE_SW0 = 0x00u;
    VID_COMPARE_SW2 = 0x00u;
    VID_COMPARE_SW3 = tmpSW3 & (uint8)(~VID_COMPARE_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = VID_COMPARE_CR;
    VID_COMPARE_CR |= (VID_COMPARE_CAL_ON | VID_COMPARE_HYST_OFF);
    ...
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        VID_COMPARE_TR0 = VID_COMPARE_DEFAULT_CMP_TRIM;
        VID_COMPARE_TR1 = VID_COMPARE_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (VID_COMPARE_CR & VID_COMPARE_PWR_MODE_MASK) == VID_COMPARE_PWR_MODE_FAST)
    {
        VID_COMPARE_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        VID_COMPARE_trimAdjust(1u);
		VID_COMPARE_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    VID_COMPARE_CR = tmpCR;
    
    /* Restore routing registers for inP */
    VID_COMPARE_SW0 = tmpSW0;
    VID_COMPARE_SW2 = tmpSW2;
    VID_COMPARE_SW3 = tmpSW3;
    ...
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)VID_COMPARE_TR1 << 8u) | (VID_COMPARE_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: VID_COMPARE_LoadTrim
********************************************************************************
*
* Summary:
...
void VID_COMPARE_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: VID_COMPARE_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: VID_COMPARE_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
