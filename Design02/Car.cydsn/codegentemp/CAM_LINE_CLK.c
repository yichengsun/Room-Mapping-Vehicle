...
/*******************************************************************************
* Function Name: CAM_LINE_CLK_Start
********************************************************************************
*
* Summary:
...
void CAM_LINE_CLK_Start(void) 
{
    ...
    CAM_LINE_CLK_CLKEN |= CAM_LINE_CLK_CLKEN_MASK;
	CAM_LINE_CLK_CLKSTBY |= CAM_LINE_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CAM_LINE_CLK_Stop
********************************************************************************
*
* Summary:
...
void CAM_LINE_CLK_Stop(void) 
{
    ...
    CAM_LINE_CLK_CLKEN &= (uint8)(~CAM_LINE_CLK_CLKEN_MASK);
	CAM_LINE_CLK_CLKSTBY &= (uint8)(~CAM_LINE_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CAM_LINE_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CAM_LINE_CLK_CLKEN & CAM_LINE_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CAM_LINE_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CAM_LINE_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CAM_LINE_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CAM_LINE_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CAM_LINE_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CAM_LINE_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CAM_LINE_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CAM_LINE_CLK_GetSourceRegister();
    uint16 oldDivider = CAM_LINE_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CAM_LINE_CLK_CLKEN & CAM_LINE_CLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CAM_LINE_CLK_DIV_PTR, clkDivider);
                CAM_LINE_CLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CAM_LINE_CLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CAM_LINE_CLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CAM_LINE_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CAM_LINE_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CAM_LINE_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: CAM_LINE_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CAM_LINE_CLK_MOD_SRC |= modeBitMask & (uint8)CAM_LINE_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: CAM_LINE_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CAM_LINE_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CAM_LINE_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: CAM_LINE_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CAM_LINE_CLK_MOD_SRC & (uint8)(CAM_LINE_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: CAM_LINE_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CAM_LINE_CLK_GetDividerRegister();
    uint8 oldSrc = CAM_LINE_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CAM_LINE_CLK_MOD_SRC |= CYCLK_SSS;
        CAM_LINE_CLK_MOD_SRC =
            (CAM_LINE_CLK_MOD_SRC & (uint8)(~CAM_LINE_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CAM_LINE_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CAM_LINE_CLK_MOD_SRC & CAM_LINE_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CAM_LINE_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CAM_LINE_CLK_PHASE = clkPhase & CAM_LINE_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CAM_LINE_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CAM_LINE_CLK_PHASE & CAM_LINE_CLK_PHASE_MASK;
}
