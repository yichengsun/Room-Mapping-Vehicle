...
uint32 HE_TIMER_ReadPeriod(void) 
{
   ...
   #else
       return (CY_GET_REG32(HE_TIMER_PERIOD_LSB_PTR));
   #endif /* (HE_TIMER_UsingFixedFunction) */
}
...
void HE_TIMER_WritePeriod(uint32 period) 
{
    ...
    #else
        CY_SET_REG32(HE_TIMER_PERIOD_LSB_PTR, period);
...
uint32 HE_TIMER_ReadCapture(void) 
{
   ...
   #else
       return (CY_GET_REG32(HE_TIMER_CAPTURE_LSB_PTR));
   #endif /* (HE_TIMER_UsingFixedFunction) */
}
...
void HE_TIMER_WriteCounter(uint32 counter) 
{
    ...
    #else
        CY_SET_REG32(HE_TIMER_COUNTER_LSB_PTR, counter);
